...
public class F_AdvancedStreams {
    ...
    private static Stream<String> words(BufferedReader reader) {
        return reader.lines().flatMap(SPLIT_PATTERN::splitAsStream);
    }
    ...
    public void f1_mapLengthToWordList() throws IOException {
        Map<Integer, List<String>> result = words(reader)
    ...
    public void f2_mapLengthToWordCount() throws IOException {
        Map<Integer, Long> result = words(reader)
    ...
    public void f3_wordFrequencies() throws IOException {
        Map<String, Long> result = words(reader)
...
//                            Long::sum));
    ...
    public void f4_nestedMaps() throws IOException {
        Map<String, Map<Integer, List<String>>> result = words(reader)
                .collect(groupingBy(s -> String.valueOf(s.charAt(0)),
    ...
    public void f5_separateOddEvenSums() {
        ...
        Map<Boolean, Integer> sums = input.boxed()
...
//                .parallelStream()
        ...
        BiConsumer<StringBuilder, String> accumulator = (sb, s) -> sb.insert(0, s).append(s);
        BiConsumer<StringBuilder, StringBuilder> combiner = (a, b) -> {
            ...
            String right = b.substring(mid);
            a.insert(0, left).append(right);
        };

        String result = input.collect(supplier, accumulator, combiner).toString();
    ...
    public void f7_countTotalAndDistinctWords() {
        ...
        TotalAndDistinct totalAndDistinct =
                Collections.nCopies(1_000, allWords)
                        .stream() // as always, takes less time than `parallelStream`
//                        .parallelStream()
                        .flatMap(List::stream)
        ...
        assertEquals("total count", 107_000, totalAndDistinct.getTotalCount());
    }
